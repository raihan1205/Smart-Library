...
before final submission are the **unused/misplaced `LibraryADT` interface**, the
**leftover demo `main` and `[cite:]` tags**, and the absence of **book-return /
availability** logic. Resolving the high-priority items would make the project both
compile cleanly and present as finished, production-quality coursework.
